...
-- FIX: Admission data not saving to Supabase

-- Run this in Supabase SQL Editor if you already ran older schemas

-- Then run separate-document-columns.sql for 4 separate doc columns
...
-- Separate document file paths (one column per required document)
...
-- Fix payment_status constraint to allow 'submitted'

ALTER TABLE admission_applications DROP CONSTRAINT IF EXISTS admission_applications_payment_status_check;

ALTER TABLE admission_applications

  ADD CONSTRAINT admission_applications_payment_status_check

  CHECK (payment_status IN ('pending', 'submitted', 'paid', 'failed', 'waived'));



-- Complete submit function (matches server.py payload)
...
-- Pending list with payment info and separate document paths
